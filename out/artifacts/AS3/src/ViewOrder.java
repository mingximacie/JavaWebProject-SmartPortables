...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("Order")!=null && request.getParameter("Order").equals("ViewOrder"))
		{
			...
			{	
				...
				{
					pw.print("<tr>");			
					pw.print("<td><input type='radio' name='orderName' value='"+oi.getProductId()+"'></td>");
					pw.print("<td>"+oi.getOrderId()+".</td><td>"+oi.getUserName()+"</td><td>"+oi.getProductId()+"</td><td>Price: "+oi.getOrderPrice()+"</td>");
		...
		if(request.getParameter("Order")!=null && request.getParameter("Order").equals("CancelOrder"))
		{
			...
			if(request.getParameter("orderName") != null)
			{
				...
				for (OrderPayment oi : orderPayments.get(orderId)) 
					{
							if(oi.getProductId().equals(orderName))
							{
								...
								for(Map.Entry<String,Product> p: prod.entrySet()) {
									if (p.getValue().getName().equals(orderName)) {
										Product product = p.getValue();
										MySqlDataStoreUtilities.updateproducts(product.getType(), product.getId(), orderName,
